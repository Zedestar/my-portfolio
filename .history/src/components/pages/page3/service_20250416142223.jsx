        ...
        <div className="h-full w-full flex flex-col gap-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 m-4">
            {ourService.map((service, index) => (
              <div
                key={index}
                className="custom-green p-4 bg-custom-green rounded-lg w-auto h-72 space-y-2 shadow-lg text-white hover:shadow-2xs transition"
              >
                {service.icon}
                <p className="text-4xl font-bold">{service.service}</p>
